...
response = requests.get('http://www.sfu.ca/outlines.html?2019/spring/cmpt/276/d100')
...
def room_parser(room, time_day):
    ...
    return [room_info, time_start, time_end]
...
room = re.search('(.*)\s\d+', room)[0]  # anything followed by space followed by one or more digits
name = re.search('(?<=-\s)(.*)$', name)[0]  # match everything after '- '
time_start = re.search('\d+\:\d+\s[a-zA-Z]+', time_day)[0]
time_end = re.search('(?<=– )(.*)', time_day)[0]  # match everything after pesky en dash!
